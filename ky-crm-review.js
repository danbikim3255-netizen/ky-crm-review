  ...
  async function handleReviewClick() {
    ...
    try {
      ...
      if (missingImgs.length > 0) {
        _dbg(`[IMG] AI가 ${missingImgs.length}개 이미지 플레이스홀더 누락 — 참고자료 끝에 강제 추가`);
        const imgBlock = missingImgs.map(tag => `<p style="margin:0;">${tag}</p>`).join("");
        finalHtml += `<p style="margin:0;">&nbsp;</p><p style="margin:0;"><b><span style="color:#FF6F00;">━━━━━━ 첨부 이미지 ━━━━━━</span></b></p>${imgBlock}`;
      }
