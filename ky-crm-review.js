// KY CRM Case Review Bookmarklet v4.17.1
...
6. **이미지 플레이스홀더 필수 유지**: 원문에 [IMAGE_1], [IMAGE_2] 등이 있으면 리뷰 출력에서 반드시 **원문과 동일한 순서로** 포함하세요. [IMAGE_1]이 [IMAGE_2]보다 먼저 나와야 합니다. 각 플레이스홀더는 원문에서 해당 이미지가 위치한 맥락과 같은 위치에 배치하세요. 플레이스홀더를 빠뜨리거나 순서를 바꾸면 이미지가 잘못 표시됩니다.
...
핵심 문제
이 케이스의 근본 원인 또는 가장 중요한 에러를 1~2문장으로. R&D가 이것만 읽고 이슈를 파악할 수 있어야 합니다.

상세 내용
Branch Office 원문 내용과 로그 분석 요약을 간결하게 나열합니다. 반복·부연 설명 제거. 한 항목당 1~2줄 이내. 기술 용어는 영문 유지합니다.
...
8. **메타 표현 금지**: "추가로 확인된", "로그에서 발견된", "분석 결과", "확인됨" 같은 부연 표현 금지. 에러 내용을 바로 쓰세요.`;
  ...
  function buildEnhancedPrompt(branchText, linkedContent, hasImages, dateStr, zipImageNames, existingHqReview) {
    ...
    if (linkedContent.length > 0) {
      ...
      const failedLinks = skippedItems.filter(item => item.url);
      if (failedLinks.length > 0) {
        const failedInfo = failedLinks.map(item => `- ${item.text || "첨부파일"}: ${item.url}`).join("\n");
        linkedSection += `\n\n--- 첨부 링크 (접근 불가) ---\n아래 첨부 파일은 자동 다운로드가 불가하여 내용을 직접 분석할 수 없었습니다.\n${failedInfo}\n--- 끝 ---\n\n[필수 지시] 위 첨부 링크에 대해 반드시 "참고 자료" 섹션에 다음 정보를 포함하세요:\n- 각 파일명과 링크\n- 파일명에서 유추 가능한 내용 (예: CheckupTool → 장비 진단 로그, ErrorLog → 에러 기록)\n- "자동 다운로드 불가로 내용 미분석 — 수동 확인 필요" 문구`;
      }
    }
  ...
  async function handleReviewClick() {
    ...
    try {
      ...
      let reviewHtml = buildReviewHtml(rawText, zipImages);
      removeModal();
