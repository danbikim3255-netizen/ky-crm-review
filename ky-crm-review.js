...
4. **이슈의 원인 규명에 직접 도움이 되는 발견만** 작성. 이슈와 무관한 설정값, 정상 동작 내역, 단순 존재 확인, 타임스탬프 나열은 모두 제외.
5. 각 항목은 "• "로 시작, 1줄로 간결하게. 인과관계가 있으면 다음 줄에 "→ " 접두어로 결론 기술.
6. **이미지**: 중복 제거 후 대표 이미지만 표시. [ZIP_IMG_N] 플레이스홀더 + 관찰 현상 1줄.
7. 권장사항, 코멘트 금지.
8. **메타 표현 금지**: "추가로 확인된 에러", "로그에서 발견된", "분석 결과 확인된" 같은 부연 표현 금지. 에러 내용을 바로 쓰세요.`;
  ...
  function buildReviewHtml(reviewText, zipImages) {
    ...
    return reviewText.split("\n").map((line) => {
      ...
      if (line.startsWith("[첨부 문서 분석]") || line.startsWith("[첨부문서분석]")) { inLinkedSection = true; return `<p style="margin:8px 0 4px;"><span style="color:#FF6F00;font-weight:bold;font-size:13px;">━━━━━━ 참고자료 파일 분석 ━━━━━━</span></p>`; }
      if (line.startsWith("[첨부 분석 끝]") || line.startsWith("[첨부분석끝]")) { inLinkedSection = false; return ""; }
      if (inLinkedSection) {
        if (line.startsWith("[파일:") || line.startsWith("[PDF:") || line.startsWith("[폴더:") || line.startsWith("[Log") || line.startsWith("[ZIP")) return `<p style="margin:8px 0 2px;padding:3px 8px;background:#FFF3E0;border-left:3px solid #FF6F00;"><b><span style="color:#E65100;font-size:12px;">${linkifyUrls(line.replace(/^\[|\]$/g, ""))}</span></b></p>`;
        if (line.trim() === "") return "";
        if (line.trim().startsWith("•")) return `<p style="margin:1px 0;padding-left:16px;">${linkifyUrls(line.trim())}</p>`;
        if (line.trim().startsWith("→")) return `<p style="margin:1px 0;padding-left:32px;">${linkifyUrls(line.trim())}</p>`;
        return `<p style="margin:1px 0;padding-left:16px;">${linkifyUrls(line)}</p>`;
      }
