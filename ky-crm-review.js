// KY CRM Case Review Bookmarklet v4.13.0
...
6. **이미지 플레이스홀더 필수 유지**: 원문에 [IMAGE_1], [IMAGE_2] 등이 있으면 리뷰 출력에서 관련 내용 바로 아래에 반드시 그대로 포함하세요. 플레이스홀더를 빠뜨리면 이미지가 리뷰에서 사라집니다.
6. **가독성**: 섹션 소제목 앞에 빈 줄을 넣으세요. 긴 문장은 의미 단위로 줄바꿈하세요. 한 줄에 하나의 정보만 담으세요.
7. **출처 태그 (필수)**: 첨부 로그에서 확인한 사실을 쓸 때는 해당 항목 **맨 앞에** 대괄호 태그를 붙이세요. 태그는 로그 종류의 짧은 이름을 사용합니다. 예: [CheckupTool], [KSMART], [로그파일명]. 핵심 문제·상세 내용 섹션에서 로그 기반 내용은 반드시 이 태그로 시작해야 합니다. Branch Office 원문 기반 내용(태그 불필요)과 로그 분석 내용(태그 필수)을 명확히 구분하세요.
...
핵심 문제
이 케이스의 근본 원인 또는 가장 중요한 에러를 1~2문장으로. R&D가 이것만 읽고 이슈를 파악할 수 있어야 합니다. 첨부 로그에서 확인한 에러라면 문장 맨 앞에 [로그명] 태그를 붙이세요. 예: "[CheckupTool] ErrLog에서 Wrong response header 에러 지속 발생." 분석한 로그가 여러 개면 각 로그의 핵심 발견을 태그와 함께 별도 줄로 나열하세요.

상세 내용
Branch Office 원문 내용과 로그 분석 요약을 간결하게 나열합니다. 원문 기반 항목은 태그 없이 작성하고, 로그에서 확인한 항목은 맨 앞에 [로그명] 태그를 붙여 출처를 명확히 하세요. 예: "[KSMART] deploy-web.log에서 패치 적용 시점 확인." 반복·부연 설명 제거. 한 항목당 1~2줄 이내. 기술 용어는 영문 유지합니다.
...
2. 모든 파일 정상이면 섹션 자체 생략.
3. **소스별 그룹핑 (절대 필수)**: 첨부 데이터가 "=== 소스 N: 이름 ===" 블록으로 구분되어 있으면, 반드시 **매 블록마다** "[소스: 이름]" 헤더를 한 줄로 출력하고 그 아래에 발견 항목을 나열하세요. 예: "[소스: 152646_CheckupTool.Zip]", "[소스: ksmart-log.zip]". "[파일:]" 헤더 사용 금지. 소스가 하나뿐이면 헤더 없이 바로 작성. **소스가 2개 이상이면 [소스:] 헤더를 빠뜨리면 안 됩니다.**
4. **각 소스별로** 케이스 이슈의 직접 원인이거나 원인 파악 단서가 되는 내용만 작성 (소스당 최대 3~5개). 발견사항이 없는 소스도 "[소스: 아카이브명]" 헤더 + "• 이슈 관련 에러 없음" 한 줄로 명시. 제외 대상: 이슈와 무관한 에러/경고, 일반적 설정값, 정상 동작 내역, 단순 존재 확인, 타임스탬프 나열, 반복되는 동일 에러(대표 1개만), 환경 정보 단순 나열.
5. 각 항목은 "• "로 시작, 1줄로 간결하게. 인과관계가 있으면 바로 다음 줄에 "→ " 접두어로 결론 기술. 각 "• " 항목(및 연결된 "→ ") 사이에 빈 줄 하나를 넣어 시각적으로 구분.
6. **이미지**: 중복 제거 후 대표 이미지만 표시. [ZIP_IMG_N] 플레이스홀더 + 관찰 현상 1줄.
7. 권장사항, 코멘트, 부연 설명 금지.
8. **메타 표현 금지**: "추가로 확인된", "로그에서 발견된", "분석 결과", "확인됨" 같은 부연 표현 금지. 에러 내용을 바로 쓰세요.`;
  ...
  async function fetchSharePointViaProxyGraphInfo(link) {
    try {
      ...
      if (!status.authenticated) {
        _dbg("[SP] 프록시 미인증 — 자동 인증 시작");
        try { await fetch(`${CORS_PROXY_URL}/auth/start`, { signal: AbortSignal.timeout(3000) }); } catch {}
        try { updateLoadingMessage("Microsoft 로그인이 필요합니다. 브라우저에서 로그인해주세요..."); } catch {}
        for (let i = 0; i < 60; i++) {
          await new Promise(r => setTimeout(r, 2000));
          try {
            const checkResp = await fetch(`${CORS_PROXY_URL}/auth/status`, { signal: AbortSignal.timeout(3000) });
            if (checkResp.ok) { const s = await checkResp.json(); if (s.authenticated) { _dbg("[SP] 프록시 인증 완료!"); break; } }
          } catch {}
        }
        const recheckResp = await fetch(`${CORS_PROXY_URL}/auth/status`, { signal: AbortSignal.timeout(3000) });
        if (!recheckResp.ok) return null;
        const recheck = await recheckResp.json();
        if (!recheck.authenticated) { _dbg("[SP] 프록시 인증 타임아웃"); return null; }
      }

      _dbg(`[SP] 프록시 Graph API 조회: ${link.url.substring(0, 80)}...`);
      try { updateLoadingMessage("SharePoint 파일 목록을 가져오고 있습니다..."); } catch {}

      const infoResp = await fetch(`${CORS_PROXY_URL}/graph-info?shareUrl=${encodeURIComponent(link.url)}`, { signal: AbortSignal.timeout(30000) });
      if (!infoResp.ok) {
        const errBody = await infoResp.text().catch(() => "");
        _dbg(`[SP] 프록시 Graph API 실패: HTTP ${infoResp.status} ${errBody.substring(0, 100)}`);
        if (infoResp.status === 401) {
          _dbg("[SP] 프록시 토큰 만료 — 재인증 필요");
          try { await fetch(`${CORS_PROXY_URL}/auth/start`, { signal: AbortSignal.timeout(3000) }); } catch {}
        }
        return null;
      }
      ...
      let fileList;
      if (info.type === "folder") {
        fileList = (info.children || []).map(f => ({ name: f.name, size: f.size, url: f.downloadUrl }));
        _dbg(`[SP] Graph API 폴더: ${fileList.length}개 파일`);
      } else {
        fileList = [{ name: info.name, size: info.size, url: info.downloadUrl }];
        _dbg(`[SP] Graph API 단일 파일: ${info.name}`);
      }

      return await processSharePointFileList(link, fileList);
    } catch (err) {
      _dbg(`[SP] 프록시 Graph API 예외: ${err.message}`);
      return null;
    }
  }
  ...
  async function fetchSharePointFolder(link) {
    ...
    if (await checkProxy()) {
      try {
        const graphResult = await fetchSharePointViaProxyGraphInfo(link);
        if (graphResult) { _dbg("[SP] 프록시 Graph API 성공"); return graphResult; }
      } catch (err) { _dbg(`[SP] 프록시 Graph API 예외: ${err.message}`); }
    }
  ...
  async function fetchExternalFile(link) {
    try {
      ...
      if (isWeTransfer && await checkProxy()) {
        ...
        const tN = allE.filter((e) => /\.(log|txt|csv|ini|cfg|conf|xml|json|dat|rsl|rpt)$/i.test(e.name) && !e.name.endsWith("/"));
        const iN = allE.filter((e) => getImageMediaType(e.name) && !e.name.endsWith("/"));
        let wtDisplayName = link.text; try { const p = decodeURIComponent(new URL(link.url).pathname); const fn = p.split("/").filter(Boolean).pop(); if (fn) wtDisplayName = fn.replace(/\?.*$/, ""); } catch {}
        let zipSum = `WeTransfer ZIP "${wtDisplayName}" (총 ${allE.length}개 파일)`;
        if (tN.length > 0) { zipSum += `\n  텍스트/로그 (${tN.length}개):`; for (const e of tN.slice(0, 30)) zipSum += `\n    - ${e.name} (${Math.round(e.uncompSize / 1024)}KB)`; }
        if (iN.length > 0) zipSum += `\n  이미지: ${iN.length}개`;
        let content = zipSum;
        for (const f of (data.files || [])) content += `\n\n--- ZIP 내부 텍스트: ${wtDisplayName} > ${f.name} ---\n${f.text}\n--- 끝 ---`;
      ...
      if (isDropbox && await checkProxy()) {
        ...
        const tN = allE.filter((e) => /\.(log|txt|csv|ini|cfg|conf|xml|json|dat|rsl|rpt)$/i.test(e.name) && !e.name.endsWith("/"));
        const iN = allE.filter((e) => getImageMediaType(e.name) && !e.name.endsWith("/"));
        let zipDisplayName = link.text; try { const p = decodeURIComponent(new URL(link.url).pathname); const fn = p.split("/").filter(Boolean).pop(); if (fn) zipDisplayName = fn.replace(/\?.*$/, ""); } catch {}
        let zipSum = `Dropbox ZIP "${zipDisplayName}" (총 ${allE.length}개 파일)`;
        if (tN.length > 0) { zipSum += `\n  텍스트/로그 (${tN.length}개):`; for (const e of tN.slice(0, 30)) zipSum += `\n    - ${e.name} (${Math.round(e.uncompSize / 1024)}KB)`; }
        if (iN.length > 0) zipSum += `\n  이미지: ${iN.length}개`;
        let content = zipSum;
        for (const f of (data.files || [])) content += `\n\n--- ZIP 내부 텍스트: ${zipDisplayName} > ${f.name} ---\n${f.text}\n--- 끝 ---`;
        const zipImages = (data.images || []).map((img) => ({ name: img.name, base64: img.base64, mediaType: img.mediaType, zipName: link.text }));
      ...
      if (isZip) {
        ...
        let extZipName = link.text; try { const p = decodeURIComponent(new URL(link.url).pathname); const fn = p.split("/").filter(Boolean).pop(); if (fn) extZipName = fn.replace(/\?.*$/, ""); } catch {}
        let content = "";
        for (const tr of zipResult.textResults) content += `\n\n--- ZIP 내부 텍스트: ${extZipName} > ${tr.name} ---\n${tr.text}\n--- 끝 ---`;
        const allE = zipResult.allEntries;
        const tN = allE.filter((e) => /\.(log|txt|csv|ini|cfg|conf|xml|json|dat|rsl|rpt)$/i.test(e.name) && !e.name.endsWith("/"));
        const iN = allE.filter((e) => getImageMediaType(e.name) && !e.name.endsWith("/"));
        let zipSum = `외부 ZIP "${extZipName}" (총 ${allE.length}개 파일)`;
        if (tN.length > 0) { zipSum += `\n  텍스트/로그 (${tN.length}개):`; for (const e of tN.slice(0, 30)) zipSum += `\n    - ${e.name} (${Math.round(e.uncompSize / 1024)}KB)`; }
        if (iN.length > 0) zipSum += `\n  이미지: ${iN.length}개`;
        content = `${zipSum}${content}`;
  ...
  function _normalizeUrl(url) { try { const u = new URL(url); if (/dropbox\.com|dropboxusercontent\.com/i.test(u.hostname)) return u.origin + u.pathname; } catch {} return url; }
  function extractLinks(editor) {
    ...
    for (const a of anchors) {
      let href = a.getAttribute("href"); if (!href || href === "#") continue; if (href.startsWith("/")) href = origin + href; const normHref = _normalizeUrl(href); if (seen.has(normHref)) continue; seen.add(normHref); seen.add(href);
    ...
    while ((urlMatch = urlRegex.exec(allText)) !== null) {
      let url = urlMatch[0].replace(/[.,;:!?]+$/, ""); const normUrl = _normalizeUrl(url); if (seen.has(normUrl)) continue; seen.add(normUrl); seen.add(url);
  ...
  function buildEnhancedPrompt(branchText, linkedContent, hasImages, dateStr, zipImageNames, existingHqReview) {
    ...
    if (linkedContent.length > 0) {
      ...
      const validItems = linkedContent.filter(item => {
        ...
        if (SKIP_EXTS.test(item.text)) { _skipped.push(`${item.text} (바이너리 파일 제외)`); return false; }
        return true;
      });
      const skippedItems = linkedContent.filter(item => (item.error && !item.content));
      skippedItems.forEach(item => _skipped.push(`${item.text} (${item.error})`));
      const perSourceBudget = validItems.length > 0 ? Math.floor(MAX_TOTAL_LINKED_CHARS / validItems.length) : MAX_TOTAL_LINKED_CHARS;
      _dbg(`[프롬프트] 유효 소스 ${validItems.length}개, 소스당 예산 ${Math.round(perSourceBudget / 1024)}KB`);
      const parts = []; let sourceIdx = 0;
      for (const item of validItems) {
        let content = item.content;
        if (content.length > perSourceBudget) { _truncated.push(`${item.text} (${Math.round(item.content.length / 1024)}KB → ${Math.round(perSourceBudget / 1024)}KB)`); content = content.substring(0, perSourceBudget) + `\n... (소스당 예산 초과 — 일부만 포함)`; }
        let displayName = item.text;
        if (item.type === "external" && /^https?:\/\//.test(item.text)) { try { const urlPath = decodeURIComponent(new URL(item.text).pathname); const fn = urlPath.split("/").filter(Boolean).pop(); if (fn) displayName = fn.replace(/\?.*$/, ""); } catch {} }
        sourceIdx++;
        parts.push(`=== 소스 ${sourceIdx}: ${displayName} ===\n${content}\n=== 소스 ${sourceIdx} 끝 ===`);
        _analyzed.push(`소스${sourceIdx}: ${displayName} (${Math.round(content.length / 1024)}KB)`);
        _dbg(`[프롬프트] 소스${sourceIdx}: "${displayName}" = ${Math.round(content.length / 1024)}KB`);
      }
      const sourceNames = validItems.map((it, i) => parts[i].match(/=== 소스 \d+: (.+?) ===/)?.[1] || it.text);
      linkedSection = parts.length > 1
        ? `\n\n--- 첨부된 문서/리소스 내용 (총 ${sourceIdx}개 소스) ---\n\n${parts.join("\n\n")}\n\n--- 첨부 내용 끝 ---\n\n[절대 필수 지시] 위 첨부 내용은 ${sourceIdx}개 소스로 구분되어 있습니다.\n[첨부 문서 분석] 섹션에서 반드시 아래 형식을 지키세요:\n${sourceNames.map((name, i) => `  ${i + 1}. "[소스: ${name}]" ← 이 헤더를 반드시 한 줄로 출력한 뒤, 그 아래에 해당 소스의 에러/이슈를 나열`).join("\n")}\n\n[소스:] 헤더가 하나라도 빠지면 잘못된 출력입니다. ${sourceIdx}개 소스 모두 "[소스: 이름]" 헤더가 있어야 합니다.`
        : `\n\n--- 첨부된 문서/리소스 내용 ---\n\n${parts.join("\n\n")}\n\n--- 첨부 내용 끝 ---`;
    }
  ...
  async function callApi(content) {
    ...
    return { ok: false, status: 529, body: "API 서버 과부하가 지속됩니다. 잠시 후 다시 시도해주세요." };
  }
  ...
  async function handleReview(branchText, hasImages, pdfData, textFileData, sharepointLinks, zipData, externalLinks, nasLinks, existingHqReview) {
    ...
    return { text: result.text, zipImages: usedImages.map((img) => ({ name: img.name, base64: img.base64, mediaType: img.mediaType })), sourceNames: _sourceNames, _debug: _debugLog, _fileStats: buildResult.fileStats };
  }
  ...
  function injectSourceTags(text, sourceNames) {
    if (!sourceNames || sourceNames.length === 0) return text;
    const kwMap = {};
    for (const name of sourceNames) {
      const lower = name.toLowerCase();
      const keys = [];
      if (/checkup/i.test(lower)) keys.push("checkuptool", "checkup", "errlog", "wrong response header");
      if (/ksmart/i.test(lower)) keys.push("ksmart", "deploy-web", "deploy_web", "패치");
      if (/spi/i.test(lower)) keys.push("spi");
      if (/aoi/i.test(lower)) keys.push("aoi");
      if (/meister/i.test(lower)) keys.push("meister");
      if (/neptune/i.test(lower)) keys.push("neptune");
      if (/zenith/i.test(lower)) keys.push("zenith");
      if (keys.length === 0) {
        const base = name.replace(/\.(zip|rar|7z|tar|gz|pdf|txt|log)$/i, "").replace(/^\d+_/, "");
        if (base) keys.push(base.toLowerCase());
      }
      const shortTag = name.replace(/\.(zip|rar|7z|tar|gz)$/i, "").replace(/^\d+_/, "") || name;
      for (const k of keys) kwMap[k] = shortTag;
    }
    const lines = text.split("\n");
    let inTopSection = false;
    const topSections = ["핵심 문제", "상세 내용", "상세내용"];
    const endSections = ["장비", "환경", "조치", "참고", "첨부", "[첨부"];
    const result = [];
    const bannerLine = `[분석 소스: ${sourceNames.join(", ")}]`;
    let bannerInserted = false;
    for (const line of lines) {
      const trimmed = line.trim();
      if (trimmed.startsWith("[TS HQ Reviewed by AI")) {
        result.push(line);
        if (!bannerInserted && sourceNames.length > 0) { result.push(bannerLine); bannerInserted = true; }
        continue;
      }
      if (topSections.some(s => trimmed.startsWith(s))) { inTopSection = true; result.push(line); continue; }
      if (endSections.some(s => trimmed.startsWith(s))) { inTopSection = false; result.push(line); continue; }
      if (inTopSection && trimmed.length > 0 && !trimmed.startsWith("[") && !/^[-=]+$/.test(trimmed)) {
        let matched = null;
        const lineLower = trimmed.toLowerCase();
        for (const [kw, tag] of Object.entries(kwMap)) {
          if (lineLower.includes(kw)) { matched = tag; break; }
        }
        if (matched) { result.push(`[${matched}] ${trimmed}`); continue; }
      }
      result.push(line);
    }
    return result.join("\n");
  ...
  function buildReviewHtml(reviewText, zipImages) {
    ...
    return reviewText.split("\n").map((line) => {
      ...
      if (line.startsWith("[첨부 문서 분석]") || line.startsWith("[첨부문서분석]")) { inLinkedSection = true; return `<p style="margin:0;">&nbsp;</p><p style="margin:0;"><span style="color:#FF6F00;font-weight:bold;">━━━━━━ 참고자료 파일 분석 ━━━━━━</span></p><p style="margin:0;">&nbsp;</p>`; }
      if (line.startsWith("[첨부 분석 끝]") || line.startsWith("[첨부분석끝]")) { inLinkedSection = false; return ""; }
      if (inLinkedSection) {
        if (line.startsWith("[파일:") || line.startsWith("[소스:") || line.startsWith("[PDF:") || line.startsWith("[폴더:") || line.startsWith("[Log") || line.startsWith("[ZIP")) return `<p style="margin:0;">&nbsp;</p><p style="margin:0;padding:3px 8px;background:#FFF3E0;border-left:3px solid #FF6F00;"><b><span style="color:#E65100;">${linkifyUrls(line.replace(/^\[|\]$/g, ""))}</span></b></p>`;
      ...
      if (line.startsWith("[분석 소스:")) return `<p style="margin:0;padding:2px 8px;background:#E8F5E9;border:1px solid #4CAF50;border-radius:4px;display:inline-block;"><span style="color:#1B5E20;font-size:12px;font-weight:bold;">${line}</span></p>`;
      const trimmed = line.trim();
      ...
      if (trimmed === "") return "";
      const styledLine = linkifyUrls(line).replace(/\[([A-Za-z가-힣0-9_\-\.]+)\]/, '<span style="display:inline-block;background:#E3F2FD;border:1px solid #1976D2;border-radius:3px;padding:0 5px;color:#0D47A1;font-weight:bold;font-size:12px;margin-right:4px;">$1</span>');
      return `<p style="margin: 0;">&nbsp;&nbsp;&nbsp;&nbsp;${styledLine}</p>`;
    }).join("");
  }

  function showModal(title, content, isLoading, hasLinks, onSave, isHtml) {
    removeModal();
    ...
    const bodyEl = document.createElement("div"); bodyEl.style.cssText = "font-size:14px;line-height:1.7;color:#333;" + (isHtml ? "" : "white-space:pre-wrap;");
    if (isLoading) {
    ...
    } else if (isHtml) { bodyEl.innerHTML = content; } else { bodyEl.textContent = content; }
    ...
    if (!isLoading) {
      ...
      closeBtn.style.cssText = "padding:8px 24px;background:#888;color:white;border:none;border-radius:6px;font-size:14px;cursor:pointer;font-weight:600;";
  ...
  function createReviewButton() {
    ...
    const btn = document.createElement("button"); btn.id = BUTTON_ID; btn.dataset.source = "bookmarklet"; btn.textContent = "리뷰"; btn.title = `v${VERSION}`;
  ...
  async function handleReviewClick() {
    ...
    try {
      ...
      const rawText = reviewResult.text;
      ...
      const reviewText = injectSourceTags(rawText, reviewResult.sourceNames || []);
      ...
      for (const [placeholder, imgTag] of Object.entries(imageMap)) {
        if (finalHtml.includes(placeholder)) {
          ...
          missingImgs.push(imgTag);
        }
      }
      if (missingImgs.length > 0) {
        _dbg(`[IMG] AI가 ${missingImgs.length}개 이미지 플레이스홀더 누락 — 상세 내용 뒤에 삽입`);
        const imgBlock = missingImgs.map(tag => `<p style="margin:0;">${tag}</p>`).join("");
        const attachMarker = finalHtml.indexOf("━━━━━━ 참고자료 파일 분석");
        if (attachMarker > 0) {
          const insertAt = finalHtml.lastIndexOf("<p", attachMarker);
          finalHtml = finalHtml.substring(0, insertAt) + imgBlock + finalHtml.substring(insertAt);
        } else {
          finalHtml += imgBlock;
        }
      }
