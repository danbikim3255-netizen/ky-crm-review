...
4. **이슈의 원인 규명에 직접 도움이 되는 발견만** 작성. 이슈와 무관한 설정값, 정상 동작 내역, 단순 존재 확인("~이 정의되어 있음")은 모두 제외. 각 항목은 "• "로 시작하고 1줄로 간결하게. 인과관계가 있으면 다음 줄에 "→ " 접두어로 결론 기술.
5. **이미지**: 중복 제거 후 대표 이미지만 표시. [ZIP_IMG_N] 플레이스홀더 + 관찰 현상 1줄.
6. 권장사항, 코멘트 금지.
7. **메타 표현 금지**: "추가로 확인된 에러", "로그에서 발견된", "분석 결과 확인된" 같은 부연 표현을 쓰지 마세요. 에러 내용을 바로 쓰세요.`;
  ...
  function buildReviewHtml(reviewText, zipImages) {
    ...
    return reviewText.split("\n").map((line) => {
      ...
      if (inLinkedSection) {
        ...
        if (line.trim().startsWith("→")) return `<p style="margin: 0;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#E65100;">${linkifyUrls(line.trim())}</span></p>`;
