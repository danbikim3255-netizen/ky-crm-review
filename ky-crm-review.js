...
3. **파일별 그룹핑**: 발견사항이 있는 파일마다 "[파일: 파일명]" 헤더를 쓰고 그 아래에 발견 항목 나열. 파일이 하나뿐이면 헤더 없이 바로 작성.
4. **케이스 이슈의 직접 원인이거나 원인 파악 단서가 되는 내용만** 작성 (최대 3~5개). 제외 대상: 이슈와 무관한 에러/경고, 일반적 설정값, 정상 동작 내역, 단순 존재 확인, 타임스탬프 나열, 반복되는 동일 에러(대표 1개만), 환경 정보 단순 나열.
